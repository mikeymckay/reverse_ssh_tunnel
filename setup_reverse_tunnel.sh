...
echo "Enter port number to connect to $MIDDLEMAN_SERVER:[22]"
read MIDDLEMAN_PORT
if [[ -z $MIDDLEMAN_PORT ]]; then
        MIDDLEMAN_PORT=22
fi
...
su $SUDO_USER -c "ssh $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER -p $MIDDLEMAN_PORT \"echo I am in\""

echo "Checking to see if GatewayPorts is set on $MIDDLEMAN_SERVER"
su $SUDO_USER -c "ssh -p $MIDDLEMAN_PORT $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER \"cat /etc/ssh/sshd_config | grep 'GatewayPorts yes'\""

echo "Do you want to upload your public key to the middleman and setup public key authentication? ([y]/n)"
read COPY_KEY

if [ ! "${COPY_KEY}" = "n" ]; then
        su $SUDO_USER -c "ssh-copy-id $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER -p $MIDDLEMAN_PORT"
fi
...
AUTOSSH_GATETIME=0
su -c \"autossh -f -N -R *:\${PORT_MIDDLEMAN_WILL_LISTEN_ON}:localhost:\${MIDDLEMAN_PORT} \${MIDDLEMAN_SERVER_AND_USERNAME} -oLogLevel=error  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no\" $SUDO_USER
" > $SCRIPT_LOCATION
